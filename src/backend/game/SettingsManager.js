...
const { default: onChange } = require("on-change");
...
class Settings {
	...
	static Storage = class {
		...
		static getServerDataDirectory(targetServerName) {
		...
		static #localSettingsDirectory = null;
		...
		static #serversRootDirectory = null;
		...
		static #localDataRootDirectory = null;
		...
		static currentServerDataDirectory = null;
	...
	static fromJsonFile(incomingJsonFilePath){
	...
	static async toJsonFile(targetFilePath){
	...
	static getProperty(propertyName, propertyCategory){
	...
	static setProperty(propertyName, propertyCategory, newValue){
		// Iterate through all static classes inside of GameSettings,
		// and check if key exists
		let matchingCategory = Settings.settingsCategories.filter((catObj) => {
			if (propertyCategory == catObj.category){
...
var watchedGameSettings = onChange(Settings, (path, value, previousValue, applyData) => {
...
module.exports = watchedGameSettings;